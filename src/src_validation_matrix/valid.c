...
/*   Updated: 2024/02/15 10:37:57 by rbutzke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_valid_matrix(t_main	*var, t_valid	*v)
{
	...
	if (ft_valid_regular_line(var->v_matrix, v) <= 0)
		ft_error("Error 27! The map must contain regular lines\n", var, 2);
	if (ft_surrounded_by_walls(var->v_matrix, v) < 0)
		ft_error("Error 28! check the composition of .ber.\n", var, 2);
	if (ft_flood_fill(var->v_matrix, v) <= 0)
		ft_error("Error 29! The map cannot be completed\n", var, 2);
}
